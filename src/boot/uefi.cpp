...
void* uefi::scan_uefi_for_rsdp(void* mb2_info) {
    ...
    constexpr const EFI_GUID acpi20_guid = {0x8868E871, 0xE4F1, 0x11D3, {0xBC, 0x22, 0x00, 0x80, 0xC7, 0x3C, 0x88, 0x81}};
    
    // ACPI 1.0 GUID: EB9D2D30-2D88-11D3-9A16-0090273FC14D
    constexpr const EFI_GUID acpi10_guid = {0xEB9D2D30, 0x2D88, 0x11D3, {0x9A, 0x16, 0x00, 0x90, 0x27, 0x3F, 0xC1, 0x4D}};

    EFI_CONFIGURATION_TABLE* config_table = (EFI_CONFIGURATION_TABLE*)(sys_table->ConfigurationTable + mem::HHDM_BASE);
    acpi::sdp_descriptor* fallback_rsdp = nullptr;
    ...
    for (uint64_t i = 0; i < sys_table->NumberOfTableEntries; i++) {
        if (compare_guid(config_table[i].VendorGuid, acpi20_guid)) {
            ...
            if (acpi::SystemDescriptionPointer::is_valid_sdp(rsdp)) fallback_rsdp = rsdp;
        }
    }
